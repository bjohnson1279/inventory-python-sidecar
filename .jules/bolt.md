...
## 2026-09-12 - Monotonic Loop Properties for O(N^2) Optimization
**Learning:** When dealing with nested loops (like O(N^2) matching algorithms) where the outer loop iterates over a list sorted monotonically (e.g., descending by velocity), any target failing a monotonic condition (like `target_velocity < current_velocity`) will automatically fail for all subsequent iterations.
**Action:** Exploit sorting properties to dynamically and safely cull the inner loop search space during periodic cleanups, turning O(N^2) performance bottlenecks into nearly O(N) over time.
